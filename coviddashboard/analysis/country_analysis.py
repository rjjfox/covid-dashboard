...
def get_uk_data():
    """Pull data from UK Government API and format dataframe"""
...
def get_france_data():
    """Pull Dataframe from WHO and format"""

    df = pd.read_csv("https://covid19.who.int/WHO-COVID-19-global-data.csv")
    df = df[df.Country == "France"]

    # Drop unnecessary columns
    df = df.drop(["Country_code", "Country", "WHO_region"], axis=1)

    # Rename columns to match the uniform schema with the UK DF
    df.columns = ["date", "newCases", "cumCases", "newDeaths", "cumDeaths"]
